/**
 * Writes directive-review/checklist.md with one section per directive and one
 * checkbox per implementation file relevant to that directive.
 */
import fs from "fs";
...
const relevantFileResolvers = {
  ...
  "layer-electron": () => uniq([...electronFiles, ...sourceFilesMatching((rel) => rel.startsWith("src/gateway/services/Common/"), "src", "gateway")]),
  ...
  "create-component": () => uniq([
    ...sourceFilesMatching((rel, name) => rel.startsWith("src/app/components/") && name.endsWith(".tsx")),
    ...sourceFilesMatching((rel, name) => rel.startsWith("src/app/viewmodel/") && name.startsWith("use-") && name.endsWith("-viewmodel.ts"), "src", "app"),
    ...sourceFilesMatching((rel, name) => rel.startsWith("src/app/actions/") && (name.endsWith("-action-type.ts") || name.endsWith("-handler.ts")), "src", "app"),
  ]),
  "create-controller": () => sourceFilesMatching((rel, name) => rel.startsWith("src/app/controllers/") && name.endsWith("-controller.ts"), "src", "app"),
  "create-gateway": () => uniq([
    ...sourceFilesMatching((rel, name) => rel.startsWith("src/gateway/gateway/") && name.endsWith("-gateway.ts"), "src", "gateway"),
  ...
  "code-maintainer": () => uniq([
    ...allSourceFiles,
    path.join(repoRoot, "vayeate-theme-studio", ".agents", "skills", "code-maintainer", "scripts", "generate-code-review-checklist.mjs"),
  ]),
  "directive-maintainer": () => uniq([
    ...parseRuleNames().map((name) => path.join("vayeate-theme-studio", "AGENTS.md#" + name)),
...
function directiveKey(file) {
  ...
  if (raw.includes("#")) {
    return raw.slice(raw.indexOf("#") + 1).replace(/\.(mdc|md)$/, "");
  }
...
const directiveFiles = uniq([
  ...parseRuleNames().map((name) => `AGENTS.md#${name}`),
  ...directiveFilesUnder(".agents", "skills"),
  ...directiveFilesUnder(".claude", "skills"),
]);
