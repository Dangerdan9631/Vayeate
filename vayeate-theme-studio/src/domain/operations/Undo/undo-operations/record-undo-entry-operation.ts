...
import type { UndoFrame, UndoProcessor } from '../../../core/Undo/undo-stack-types';
import { createFrameId } from '../../../core/Undo/undo-stack-types';
import { undoManagerV2 } from '../../../core/Undo/undo-manager-v2';
import { undoEntrySchema, type UndoDiff } from '../../../../model/Undo/undo-history';
    ...
    try {
      ...
      await stack.push(entry, input.coalesceWithPrevious ? {
        ...
        merge: (existing, next) => {
          ...
          if (diffs.every((diff) => this.validateUndoDiffHasNoNetChange.test(diff))) return null;
          return {
            ...existing,
