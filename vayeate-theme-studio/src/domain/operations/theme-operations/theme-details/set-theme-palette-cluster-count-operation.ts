...
export interface ThemeNumberFieldEditResult {
  before: number;
  after: number;
  changed: boolean;
}
...
/**
 * Updates theme.paletteClusterCountK and returns before/after for undo recording.
 */
@singleton()
export class SetThemePaletteClusterCountOperation {
  constructor(
  ...
  execute(value: number): ThemeNumberFieldEditResult | null {
    const theme = this.themeUiStore.getStore().state.theme;
    if (!theme) return null;

    const before = theme.paletteClusterCountK ?? 1;
    const after = Math.max(1, Math.min(12, value));
    if (before === after) {
      return { before, after, changed: false };
    }

    const next: Theme = { ...theme, paletteClusterCountK: after };
