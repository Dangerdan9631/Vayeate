...
function undoValuesEqual(before: unknown, after: unknown, actionType?: string): boolean {
...
/**
 * Compares undo before/after values to detect no-op edits before recording history.
 */
@singleton()
export class ValidateUndoDiffHasNoNetChange {
  /**
   * @param diff - Undo diff containing action type and before/after values.
   * @returns `true` when the diff represents no net change.
   */
  test(diff: UndoDiff): boolean {
