...
import { SetMappingGroupRefOperation as SetMappingGroupRefOp } from '../../../../domain/operations/Template/template-operations/mappings/set-mapping-group-ref-operation';
...
import { TEMPLATE_MAPPING_GROUP_REF_SET } from '../../../../model/Common/undo-action-types';

/**
 * Handles TEMPLATE_MAPPING_EXISTING_TOKEN_GROUP_LIST_ON_COMMIT by assigning a mapping group.
 */
@singleton()
export class SetMappingGroupRefController {
  ...
  /**
   * Updates the group ref for a token mapping and records undo.
   * @param tokenKey Token key from the group list commit.
  ...
  async run(
    ...
    await this.recordTemplateUndo.execute({
      description: `Set ${tokenKey} mapping group`,
      actionType: TEMPLATE_MAPPING_GROUP_REF_SET,
      target: `${template.name}@${template.version}:mapping:${tokenType}:${tokenKey}:group`,
      before: template,
      after: next,
    });
  }
