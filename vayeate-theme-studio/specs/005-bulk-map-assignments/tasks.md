...
- [X] T001 Review current mapping row, semantic variant, template selection, persistence, and undo paths in `src/app/template/mappings-card/` and `src/domain/operations/Template/template-operations/mappings/`
...
- [X] T004 Extend transient mapping selection shape and mutation methods in `src/domain/state/Template/ui/template-ui-state.ts` and `src/domain/state/Template/ui/template-ui-store.ts`
- [X] T005 [P] Add selection mutation operations in `src/domain/operations/Template/template-operations/mappings/`
- [X] T006 Update template selection and unload operations to clear mapping selection in `src/domain/operations/Template/template-operations/template-list/` and `src/domain/operations/Template/template-operations/template-details/`
- [X] T007 Add complete-target validation and next-template construction tests in `src/domain/operations/Template/template-operations/mappings/apply-mapping-assignment-operation.test.ts`
- [X] T008 Implement shared all-target mapping assignment policy in `src/domain/operations/Template/template-operations/mappings/apply-mapping-assignment-operation.ts`
...
- [X] T009 [US1] Add bulk assignment action variants and routing tests in `src/app/actions/Template/mappings-card/mappings-card-action-type.ts` and `src/app/template/template-flow-routing.test.ts`
- [X] T010 [US1] Implement focused bulk assignment controller orchestration in `src/app/controllers/Template/mappings-card/apply-selected-mapping-assignment-controller.ts`
- [X] T011 [US1] Route bulk assignment actions through `src/app/actions/Template/mappings-card/mappings-card-handler.ts`
- [X] T012 [US1] Expose bulk assignment callbacks and assignment options in `src/app/viewmodel/Template/mappings-card/use-mappings-card-viewmodel.ts`
- [X] T013 [US1] Add explicit bulk group, color, and contrast controls in `src/app/components/Template/mappings-card/MappingsCard.tsx`
...
- [X] T015 [US2] Add selection action variants, controller routing, and tests in `src/app/actions/Template/mappings-card/`, `src/app/controllers/Template/mappings-card/`, and `src/app/template/template-flow-routing.test.ts`
- [X] T016 [US2] Expose selected identities, count, and named selection callbacks in `src/app/viewmodel/Template/mappings-card/use-mappings-card-viewmodel.ts`
- [X] T017 [US2] Add accessible selection affordances to `src/app/components/Template/mappings-card/MappingRow.tsx` and `src/app/components/Template/mappings-card/SemanticVariantListRow.tsx`
- [X] T018 [US2] Wire selection through grouped, semantic-base, and virtualized rendering while preserving existing row sizing changes in `src/app/components/Template/mappings-card/MappingsCard.tsx`
...
- [X] T021 [US3] Add failed, invalid, and no-op history exclusion coverage in `src/domain/operations/Common/undo-operations/undo-failure-paths.test.ts`
- [X] T022 [US3] Ensure bulk controller records one complete before/after transition and no incomplete transition in `src/app/controllers/Template/mappings-card/apply-selected-mapping-assignment-controller.ts`
