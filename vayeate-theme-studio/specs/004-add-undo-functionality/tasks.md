...
- [X] T001 Inventory current undo primitives and classify placeholder behavior in `src/domain/core/Common/undo-stack-types.ts`, `src/domain/core/Common/undo-stack.ts`, `src/domain/core/Common/undo-manager-v2.ts`, `src/domain/operations/undo-operations`, `src/gateway/gateway/Common/undo/undo-gateway.ts`, and `src/app/core/undo`
...
- [X] T005 Create typed undo context, diff, entry, stack position, transition result, and availability models in `src/model/Common/undo-history.ts`
- [X] T006 [P] Add undo model validation tests for `*Ref`-based context keys and non-snapshot diff payloads in `src/model/Common/undo-history.test.ts`
- [X] T007 Replace placeholder-only undo action typing with action-generated before/after diff action types in `src/domain/core/Common/undo-stack-types.ts`
- [X] T008 Update stack push, undo, redo, go-to, branch pruning, list, and persistence semantics in `src/domain/core/Common/undo-stack.ts`
- [X] T009 Update undo manager creation, persistence failure handling, and active-session hydration behavior in `src/domain/core/Common/undo-manager-v2.ts`
- [X] T010 [P] Update undo processor dispatch for typed reversal and reapplication actions in `src/domain/core/Common/undo-processor.ts`
- [X] T011 Update undo stack state and read-only availability summary shape in `src/domain/state/Common/undo-stack/undo-stack-state.ts` and `src/domain/state/Common/undo-stack/undo-stack-store.ts`
- [X] T012 [P] Add core undo stack tests for LIFO undo, redo order, branch pruning, 20-action depth, and go-to semantics in `src/domain/core/Common/undo-stack.test.ts`
- [X] T013 [P] Add undo manager tests for write-through persistence, memory release reload, persistence failure behavior, and startup clearing in `src/domain/core/Common/undo-manager-v2.test.ts`
- [X] T014 Update active stack selection and load operations to require context-derived stack IDs and read-only summaries in `src/domain/operations/Common/undo-operations/set-current-undo-stack-id-operation.ts` and `src/domain/operations/Common/undo-operations/load-undo-history-operation.ts`
- [X] T015 Update undo, redo, and history-go-to operations to return `HistoryTransitionResult` and preserve entries on failed transitions in `src/domain/operations/Common/undo-operations/undo-operation.ts`, `src/domain/operations/Common/undo-operations/redo-operation.ts`, and `src/domain/operations/Common/undo-operations/history-go-to-operation.ts`
- [X] T016 Update startup clearing and undo persistence adapter behavior in `src/domain/operations/Common/undo-operations/clear-persisted-undo-operation.ts` and `src/gateway/gateway/Common/undo/undo-gateway.ts`
- [X] T017 [P] Add gateway tests for saving stack changes, listing persisted stack files, clearing `data/.undo`, and handling missing files in `src/gateway/gateway/Common/undo/undo-gateway.test.ts`
- [X] T018 Update app-layer undo controller contracts for typed transition results in `src/app/core/Common/undo/undo-controller.ts`, `src/app/core/Common/undo/redo-controller.ts`, and `src/app/core/Common/undo/history-go-to-controller.ts`
...
- [X] T021 [P] [US1] Add policy tests for recording completed reversible actions and rejecting no-op actions in `src/domain/operations/Common/undo-operations/record-undo-entry-operation.test.ts`
...
- [X] T024 [US1] Implement focused undo entry recording operation for completed reversible actions in `src/domain/operations/Common/undo-operations/record-undo-entry-operation.ts`
- [X] T025 [US1] Generate before/after diffs for representative theme variable edits in `src/domain/operations/theme-operations` and `src/app/controllers/Theme/theme-variables-card`
- [X] T026 [US1] Wire theme edit controllers through the named action flow to record undo only after successful mutation in `src/app/controllers/Theme/theme-variables-card`
- [X] T027 [US1] Wire menu and keyboard undo commands to typed undo transition results in `src/app/actions/Common/menu-bar/app-menu-handler.ts`, `src/app/viewmodel/Common/menu-bar/use-menubar-viewmodel.ts`, and `src/app/controllers/Common/app-shell/handle-keyboard-shortcut-controller.ts`
- [X] T028 [US1] Update menu disabled state and next undo description from read-only availability in `src/app/components/Common/menu-bar/MenuBar.tsx` and `src/app/viewmodel/Common/menu-bar/use-menubar-viewmodel.ts`
- [X] T029 [US1] Ensure empty-stack undo is unavailable or produces no state change without false success feedback in `src/domain/operations/Common/undo-operations/undo-operation.ts` and `src/app/core/Common/undo/undo-controller.ts`
...
- [X] T030 [P] [US2] Add context-key tests for active tab plus `templateRef`, `catalogRef`, and `themeRef` values in `src/model/Common/undo-history.test.ts`
...
- [X] T033 [US2] Implement context-key derivation from active tab and existing `*Ref` values in `src/model/Common/undo-history.ts`
- [X] T034 [US2] Select the active undo stack when app shell tab state changes in `src/app/controllers/Common/app-shell` and `src/domain/operations/Common/undo-operations/set-current-undo-stack-id-operation.ts`
- [X] T035 [US2] Select the active undo stack when catalog references change in `src/app/controllers/Catalog/catalogs-card/set-selected-catalog-controller.ts` and `src/domain/state/Catalog/ui/catalog-ui-store.ts`
- [X] T036 [US2] Select the active undo stack when template references change in `src/app/controllers/Template/templates-card/select-template-and-load-controller.ts` and `src/domain/state/Template/ui/template-ui-state.ts`
- [X] T037 [US2] Select the active undo stack when theme references change in `src/app/controllers/Theme/themes-card/select-theme-and-load-controller.ts` and `src/domain/state/Theme/ui/theme-ui-state.ts`
- [X] T038 [US2] Preserve inactive stack entries during active-context undo, redo, and go-to transitions in `src/domain/core/Common/undo-manager-v2.ts` and `src/domain/core/Common/undo-stack.ts`
- [X] T039 [US2] Refresh read-only undo availability after each context switch in `src/domain/operations/Common/undo-operations/load-undo-history-operation.ts` and `src/domain/state/Common/undo-stack/undo-stack-store.ts`
...
- [X] T041 [P] [US3] Add active-session disk reload tests for memory-released stacks in `src/domain/core/Common/undo-manager-v2.test.ts`
- [X] T042 [P] [US3] Add persistence adapter failure-path tests in `src/gateway/gateway/Common/undo/undo-gateway.test.ts` and `src/domain/operations/Common/undo-operations/record-undo-entry-operation.test.ts`

### Implementation for User Story 3

- [X] T043 [US3] Ensure app startup clears prior-session undo files before undo history can be loaded in `src/app/controllers/Common/app-shell/load-app-controller.ts`
- [X] T044 [US3] Remove cross-session undo hydration from app startup while retaining active-session reload after memory release in `src/domain/operations/Common/undo-operations/load-undo-history-operation.ts`
- [X] T045 [US3] Make stack changes write through to `src/gateway/gateway/Common/undo/undo-gateway.ts` before in-memory entries are eligible for release in `src/domain/core/Common/undo-manager-v2.ts`
- [X] T046 [US3] Surface persistence failures so actions are not reported safely undoable without equivalent active-session guarantees in `src/domain/operations/Common/undo-operations/record-undo-entry-operation.ts`
...
- [X] T048 [P] [US4] Add redo and branch-pruning policy tests in `src/domain/core/Common/undo-stack.test.ts`
...
- [X] T051 [US4] Ensure redo reapplies undone entries in order in `src/domain/operations/Common/undo-operations/redo-operation.ts` and `src/domain/core/Common/undo-stack.ts`
- [X] T052 [US4] Ensure history go-to targets the state immediately after the selected item in `src/domain/operations/Common/undo-operations/history-go-to-operation.ts` and `src/domain/core/Common/undo-stack.ts`
- [X] T053 [US4] Expose ordered recent actions, current position, canUndo, and canRedo in `src/domain/state/Common/undo-stack/undo-stack-state.ts` and `src/app/viewmodel/Common/menu-bar/use-menubar-viewmodel.ts`
- [X] T054 [US4] Wire history-list item selection through named actions in `src/app/components/Common/menu-bar/MenuBar.tsx`, `src/app/actions/Common/menu-bar/app-menu-action-type.ts`, and `src/app/actions/Common/menu-bar/app-menu-handler.ts`
- [X] T055 [US4] Prune redoable branch entries when a new undoable action is recorded in `src/domain/operations/Common/undo-operations/record-undo-entry-operation.ts` and `src/domain/core/Common/undo-stack.ts`
...
- [X] T056 [P] [US5] Add failure-path policy tests for validation, cancellation, interruption, pending duplicate work, and external-detail failures in `src/domain/operations/Common/undo-operations/record-undo-entry-operation.test.ts`
...
- [X] T060 [US5] Add completed-action recording guards to catalog state-changing workflows in `src/app/controllers/Catalog/tokens-card` and `src/domain/operations/catalog-operations`
- [X] T061 [US5] Add completed-action recording guards to template state-changing workflows in `src/app/controllers/Template/variables-card`, `src/app/controllers/Template/groups-card`, and `src/domain/operations/template-operations`
- [X] T062 [US5] Add completed-action recording guards to theme state-changing workflows in `src/app/controllers/Theme/theme-palette-card`, `src/app/controllers/Theme/theme-variables-card`, and `src/domain/operations/theme-operations`
...
- [X] T064 [US5] Ensure failed undo, redo, or history navigation preserves state and targeted entries in `src/domain/operations/Common/undo-operations/undo-operation.ts`, `src/domain/operations/Common/undo-operations/redo-operation.ts`, and `src/domain/operations/Common/undo-operations/history-go-to-operation.ts`
...
- [X] T073 [P] Add `UNDO_BASELINE_FRAME_ID`, `deriveUndoBaselineLabel`, and baseline label tests in `src/model/Common/undo-history.ts` and `src/model/Common/undo-history.test.ts`
- [X] T074 [P] Add baseline goto tests in `src/domain/core/Common/undo-stack.test.ts`
- [X] T075 Expose `currentBaselineLabel`, prepend synthetic baseline rows in `refreshUndoSummary`, and set labels on context switch in `src/domain/state/Common/undo-stack/undo-stack-state.ts`, `src/domain/state/Common/undo-stack/undo-stack-store.ts`, `src/domain/operations/Common/undo-operations/undo-operation-helpers.ts`, `src/domain/operations/Common/undo-operations/set-current-undo-stack-id-operation.ts`, and `src/domain/core/Common/undo-stack.ts`
...
- [X] T077 Add baseline opened-item styling hook in `src/app/components/Common/menu-bar/MenuBar.tsx`
...
- [X] T079 [P] Add universal controller coverage enforcement in `test/architecture/undo-controller-coverage.test.ts` backed by `test/utils/undo-controller-exclusions.ts`
...
|------|-------------------|----------------|-------------------------------|
| Current undo primitives | `UndoActionNoop`, `NOOP` processor branches, `UndoFrame.actions`, stack push/undo/redo/goto/list, manager LRU hydration, `UndoGateway` stack files, app undo controllers | Migration evidence / placeholder behavior to replace | `src/domain/core/Common/undo-stack-types.ts` defines only `UndoActionNoop`; `src/domain/core/Common/undo-processor.ts` apply/revert branches are no-ops. Stack mechanics and gateway files are useful evidence, but Phase 2 must replace placeholder actions with typed action-generated before/after diffs, typed transition results, awaited write-through persistence, context-derived stack IDs, and startup clearing without cross-session restoration. |
| App shell history commands | `EditMenuUndoButtonOnClick`, `EditMenuRedoButtonOnClick`, `HistoryMenuGoToButtonOnClick`, keyboard shortcut handling | Undo infrastructure / reversible only through history transitions | `src/app/actions/Common/menu-bar/app-menu-action-type.ts` and `app-menu-handler.ts` already route menu commands to undo, redo, and history-go-to controllers. Phase 2/US1 must return and surface typed `HistoryTransitionResult`, preserve entries on failure, and keep history-list navigation policy-owned. |
| App shell context selection | `TabButtonOnClick`, active tab load/unload, selected content context changes | Context selection, not undoable by itself | `src/app/actions/Common/ribbon/app-ribbon-action-type.ts` routes tab selection through `SetActiveTabController`. Selection activates a context-scoped stack but should not create an undo entry because it changes visible navigation/context rather than reversible authored content. |
