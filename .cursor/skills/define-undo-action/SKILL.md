...
1. **UndoAction union**: Add a new member to the **UndoAction** discriminated union in `vayeate-theme-studio/src/utils/undo-manager-v2.ts`. Include `type: '<NAME>'` and the payload needed to apply and revert (e.g. apply/revert state slices or file data).
