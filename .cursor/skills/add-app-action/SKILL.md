...
1. **AppAction**: In `vayeate-theme-studio/src/app/app/actions/app-action.ts` and the matching per-domain enum file under `src/app/{app,catalog,template,theme}/actions/` (`app-action-type.ts`, `catalog-action-type.ts`, `template-action-type.ts`, or `theme-action-type.ts`), add a member to the correct enum with the string value matching the naming convention from the Action Queue rule: `<CONTEXT>_<SUBCONTEXT>_<IDENTIFIER>_<CONTROL>_<ACTION>`. Add the corresponding union member to `AppAction` as `{ type: XxxActionType.YourMember; ...payload }`. Use types from `src/model/schemas` where applicable (e.g. `CatalogName`, `Version`, `HexColor`, `TokenKey`). Dispatch sites and handler `switch` cases use the enum member, not a string literal.
...
- Handler registry: `vayeate-theme-studio/src/app/app/actions/handler-registry.ts`
