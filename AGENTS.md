...
| Adding or modifying handler routing (action → controller wiring) | `vayeate-theme-studio/src/app/handlers/handler-registry.ts` | Per-domain handler: `app-handler.ts`, `catalog-handler.ts`, `template-handler.ts`, `theme-handler.ts`; `.cursor/rules/vayeate-theme-studio-architecture.mdc` |
